...
h5bp = module.exports = function(options) {
	...
	if (options.scripts) {
		if (!options.scripts.method) {
			options.scripts.method = 'commonjs';
		}
		else if (!/(commonjs|amd)/.test(options.scripts.method)) {
			throw new Error("Script concatenation method can be either 'commonjs' or 'amd'");
		}

		if ('string' == typeof options.scripts.concat) {
			options.scripts.concat = [options.scripts.concat];
		}
		if (!Array.isArray(options.scripts.concat)) {
			throw new Error("There is no script to concatenate");
		}

		layers.push(require('./layers/' + options.scripts.method)(options));
	}
